package org.apache.dearbaby.join;
...
*/
	...
	//	sql="SELECT a.docName  FROM worker4  a left join good4 b on  a.docId=b.docId  ";
		...
		sql="SELECT a.docName  FROM worker5  a   where   a.id=100";
	//	 Date d1=new Date();
	...
	//	 Date d1=new Date();
	 	...
	 	sql="SELECT a.docName  FROM worker5  a   , good4 b  WHERE  a.docId=b.docId  and a.docId=10 ";
		//
	...
	public static void test3(){
		...
		sql="SELECT  a.docId aid,b.docId  bid   FROM worker2  a , good2 b  WHERE  a.docId=b.docId   ";
	 	sql=" SELECT  a.docId aid,b.docId  bid   FROM worker4  a , good4 b  WHERE  a.docId=b.docId and  a.docId in"
	 			+ " (select c.docId from   worker5 c ,good5 d where c.docId=d.docId and  c.docId=a.docId) ";
	  	sql="SELECT count(a.docId) cnt   FROM worker4  a , good4 b  WHERE  a.docId=b.docId and a.docId=100  ";
	 	sql="SELECT a.docName  FROM worker5  a   , good4 b  WHERE  a.docId=b.docId  and a.Id>0  and b.docId>'w0'";
		
	 	sql="SELECT a.docName  FROM worker5  a   , (select c.docId from  good4 c) b  WHERE  a.docId=b.docId  and a.Id>0  and b.docId>'w0'";
		
		DearSelector selector =  DearSelector.getSimple();  
		selector.query(sql);
	    ResultBuffer  list=selector.getResult();
	    System.out.println("sizeiiiii - "+list.size());
		while(list.hasNext()){
			 System.out.println("cnt:   - "+  list.getCurrCol("a", "docName"));
			 list.nextTo();
		 }
